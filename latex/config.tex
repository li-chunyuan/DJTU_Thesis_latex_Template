...
\graphicspath{{figures/}{./figures/}}  % 设置图片搜索路径
\usepackage{float}      % 强行不浮动 [H]
\usepackage{subcaption} % 子图支持

% 5.1 TikZ 绘图支持
\usepackage{tikz}                     % TikZ —— 强大的绘图宏包
\usetikzlibrary{positioning}          % 相对定位语法 (right=of, below=of)
\usetikzlibrary{arrows.meta}          % 丰富的箭头样式
\usetikzlibrary{shapes.geometric}     % 几何形状 (菱形、梯形等，流程图常用)
\usetikzlibrary{calc}                 % 坐标计算 ($(A)!0.5!(B)$)
